...
-- psql variables are not expanded inside PostgreSQL dollar-quoted DO blocks.
-- Keep the generated fixture identity in the disposable transaction so the
-- same value is available to both ordinary SQL and PL/pgSQL assertions.
create temporary table n4b_context (
  snapshot_a_id uuid not null
) on commit drop;

insert into n4b_context (snapshot_a_id)
values (:'snapshot_a_id'::uuid);
...
select
  (select snapshot_a_id from n4b_context),
...
insert into public.transport_source_places (
  ...
  ((select snapshot_a_id from n4b_context), 'naptan-stop-area:ROOT2', 'ROOT2', 'Root 2', 'root 2', 'NONE', '{"fixture":"A"}'),
  ((select snapshot_a_id from n4b_context), 'naptan-stop-area:ALT', 'ALT', 'Alternate Parent', 'alternate parent', 'NONE', '{"fixture":"A"}'),
  ((select snapshot_a_id from n4b_context), 'naptan-stop-area:CHILD', 'CHILD', 'Child', 'child', 'NONE', '{"fixture":"A"}');
...
join public.transport_source_places child
  on child.snapshot_id = (select snapshot_a_id from n4b_context)
 and child.publisher_identity = 'naptan-stop-area:DEEP' || i
join public.transport_source_places parent
  on parent.snapshot_id = (select snapshot_a_id from n4b_context)
...
select
  (select snapshot_a_id from n4b_context), child.id, parent.id, child.publisher_identity,
...
where child.snapshot_id = (select snapshot_a_id from n4b_context)
...
insert into public.transport_source_place_parents (
  ...
  (select snapshot_a_id from n4b_context),
  (select id from public.transport_source_places where snapshot_id = (select snapshot_a_id from n4b_context) and publisher_identity = 'naptan-stop-area:DEEP0'),
  null,
...
select
  (select snapshot_a_id from n4b_context), n.id, p.id, n.publisher_identity, p.publisher_identity,
...
where n.snapshot_id = (select snapshot_a_id from n4b_context)
  and p.snapshot_id = (select snapshot_a_id from n4b_context)
...
insert into public.transport_source_memberships (
  ...
  (select snapshot_a_id from n4b_context),
  (select id from public.transport_source_nodes where snapshot_id = (select snapshot_a_id from n4b_context) and publisher_identity = 'naptan-stop-point:TEST123'),
  null,
...
select 'snapshots', count(*) from public.transport_source_snapshots where id = (select snapshot_a_id from n4b_context)
union all select 'places', count(*) from public.transport_source_places where snapshot_id = (select snapshot_a_id from n4b_context)
union all select 'nodes', count(*) from public.transport_source_nodes where snapshot_id = (select snapshot_a_id from n4b_context)
union all select 'memberships', count(*) from public.transport_source_memberships where snapshot_id = (select snapshot_a_id from n4b_context)
union all select 'parents', count(*) from public.transport_source_place_parents where snapshot_id = (select snapshot_a_id from n4b_context);

do $$
begin
  if (select count(*) from public.transport_source_memberships where snapshot_id = (select snapshot_a_id from n4b_context) and duplicate_occurrence_count = 2) <> 1 then
    raise exception 'duplicate occurrence count was not preserved';
  end if;
  if (select count(*) from public.transport_source_memberships where snapshot_id = (select snapshot_a_id from n4b_context) and resolution_status = 'UNRESOLVED_MISSING_PLACE') <> 1 then
    raise exception 'unresolved membership was not retained';
  end if;
  if (select count(*) from public.transport_source_place_parents where snapshot_id = (select snapshot_a_id from n4b_context) and resolution_status = 'UNRESOLVED_MISSING_PARENT') <> 1 then
    raise exception 'unresolved parent edge was not retained';
  end if;
  if (select st_x(publisher_location::geometry) from public.transport_source_places where snapshot_id = (select snapshot_a_id from n4b_context) and publisher_identity = 'naptan-stop-area:DEEP0') <> -0.1200 then
    raise exception 'publisher longitude was not represented by PostGIS';
  end if;
  if (select st_y(publisher_location::geometry) from public.transport_source_places where snapshot_id = (select snapshot_a_id from n4b_context) and publisher_identity = 'naptan-stop-area:DEEP0') <> 51.5000 then
...
begin
  begin
    ...
    ) values ((select snapshot_a_id from n4b_context), 'naptan-stop-area:BAD', 'BAD', 'Bad', 91, 0, 'STOP_AREA_LEVEL');
  ...
  begin
    insert into public.transport_source_places (
      snapshot_id, publisher_identity, publisher_id, display_name,
      coordinate_scope
    ) values ((select snapshot_a_id from n4b_context), 'naptan-stop-area:DEEP0', 'DEEP0-DUP', 'Duplicate', 'NONE');
  ...
  where snapshot_id = (select snapshot_a_id from n4b_context) and publisher_identity = 'naptan-stop-area:DEEP0';
  ...
  begin
    insert into public.transport_source_memberships (
      ...
      (select snapshot_a_id from n4b_context), node_b_id, place_a_id,
  ...
  if (select count(*) from public.transport_source_places where snapshot_id = (select snapshot_a_id from n4b_context) and publisher_identity = 'naptan-stop-area:DEEP0') <> 1 then
...
where snapshot_id = (select snapshot_a_id from n4b_context)
...
begin
  ...
  select count(*) into after_count from public.transport_source_places where snapshot_id = (select snapshot_a_id from n4b_context);
  ...
  select count(*) into after_count from public.transport_source_nodes where snapshot_id = (select snapshot_a_id from n4b_context);
  ...
  select count(*) into after_count from public.transport_source_memberships where snapshot_id = (select snapshot_a_id from n4b_context);
  ...
  select count(*) into after_count from public.transport_source_place_parents where snapshot_id = (select snapshot_a_id from n4b_context);
