...
| Facility detail | FacilityDetailScreen | `screens/FacilityDetailScreen.tsx`, `services/toiletUnits.ts`, `utils/toiletUnits.ts` | Supabase facility/read, additive published `toilet_units`, and reports service | Supabase (for facility, optional unit detail, photos, reports) | BACKEND-DEPENDENT — additive model implemented, migration not deployed | Renders real facility values and explicit unknown states for hours, cost, access notes, ratings and photos; optionally renders published explicit toilet units without fabricating station-toilet coordinates or blocking the parent | Requires migration deployment and real-data smoke test; TfL canonical promotion remains unauthorized | Deploy/verify the separately approved migration before enabling live child records |
