...
- Account deletion: Profile → Delete account → explanation → typed confirmation → destructive reconfirmation. Test mode simulates only; production invokes the deployed governed `delete-account` Edge Function. Subscription-history accounts fail closed while retention handling remains unresolved.
